...
public class DownstreamDataRipplerTest {
	...
	testingUtility.getConfiguration().setStrings("target_column_family", "eg");
        ...
        otherTable = (HTable) testingUtility.createTable(otherTableName, "eg");
	...
	idxPut.addColumn("a".getBytes(), "genome_accesion_number".getBytes(), "EFB1".getBytes());
	...
	secondaryIdxTable.flushCommits();
        ...
        Assert.assertEquals( "We expect the rowkey in the rippled table to be the source rowkey", "EFG1", ck);
    }
}
